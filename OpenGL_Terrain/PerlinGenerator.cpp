...
PerlinGenerator::PerlinGenerator(unsigned repeatInterval)
...
float PerlinGenerator::gradient(int hash, float x, float y, float z)
{
...
float PerlinGenerator::lerp(float a, float b, float x)
{
    return a + x * (b - a);
}

float PerlinGenerator::perlin(float x, float y, float z)
{
    if (mRepeatInterval > 0)
    {
...
float PerlinGenerator::octavePerlin(float x, float y, float z, int octaves, float persistence)
{
    float total = 0;
    float frequency = 1; 
    float amplitude = 1;
